...
python process_tans_rois_pcf.py \
  -i "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Tumour" \
  -p "/Volumes/My Passport/Spatial_Transcriptomics_data_final/PCF/Tumour" \
  -s "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Scattered/Tumour"
...
python process_tans_rois_pcf.py \
  -i "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Peritumour" \
  -p "/Volumes/My Passport/Spatial_Transcriptomics_data_final/PCF/Peritumour" \
  -s "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Scattered/Peritumour"
...
python process_tans_rois_pcf.py \
  -i "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Healthy" \
  -p "/Volumes/My Passport/Spatial_Transcriptomics_data_final/PCF/Healthy" \
  -s "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Scattered/Healthy"
...
base = "/Volumes/My Passport/Spatial_Transcriptomics_data_final"
...
get_ipython().run_cell_magic('writefile', 'process_missing_tans_rois_pcf.py', '"""\nCompute PCFs and scatter plots for a specified list of ROIs in parallel within each slide.\n\nUsage:\n    python process_tans_rois_pcf.py \\\n      -i /path/to/input_root \\\n      -p /path/to/pcf_root \\\n      -s /path/to/scatter_root \\\n      --alpha 120\n"""\nimport os\nimport glob\nimport argparse\nimport pandas as pd\nimport numpy as np\nimport muspan as ms\nimport matplotlib.pyplot as plt\nfrom multiprocessing import Pool\n\n# Only process ROIs with these indices\nALLOWED_ROIS = {558, 568}\n\ndef process_roi(args):\n    csv_path, pcf_out_dir, scatter_out_dir, alpha = args\n    roi_name = os.path.splitext(os.path.basename(csv_path))[0]\n    df = pd.read_csv(csv_path)\n\n    # convert mm → μm\n    df[\'x_um\'] = df[\'x_mm\'] * 1000\n    df[\'y_um\'] = df[\'y_mm\'] * 1000\n\n    # build the domain\n    domain = ms.domain(roi_name)\n    coords = df[[\'x_um\', \'y_um\']].to_numpy()\n    domain.add_points(coords, \'Cell centres\')\n    domain.add_labels(\'cell_type\', df[\'cell_type\'])\n    domain.estimate_boundary(\n        method=\'alpha shape\',\n        alpha_shape_kwargs={\'alpha\': alpha}\n    )\n\n    # scatter plot of original cell types\n    os.makedirs(scatter_out_dir, exist_ok=True)\n    fig, ax = plt.subplots(figsize=(8, 6))\n    ms.visualise.visualise(domain, \'cell_type\', ax=ax, marker_size=5)\n    ax.set_title(f"{roi_name} – cell_type", pad=14)\n    fig.tight_layout(pad=2.5)\n    fig.savefig(os.path.join(scatter_out_dir, f"{roi_name}.png"), dpi=150, bbox_inches=\'tight\')\n    plt.close(fig)\n\n    # prepare original categories and pairs for PCF\n    orig_cats = df[\'cell_type\'].dropna().unique().tolist()\n    axes = [(\'cell_type\', c) for c in orig_cats]\n    pairs = [((Ai, Ci), (Aj, Cj))\n             for (Ai, Ci) in axes\n             for (Aj, Cj) in axes]\n\n    # find reference R from the first valid pair\n    r_ref = None\n    for (Ai, Ci), (Aj, Cj) in pairs:\n        try:\n            r_ref, _ = ms.spatial_statistics.cross_pair_correlation_function(\n                domain, (Ai, Ci), (Aj, Cj),\n                max_R=250, annulus_step=5, annulus_width=10,\n                exclude_zero=True, boundary_exclude_distance=10,\n                visualise_output=False\n            )\n            break\n        except ValueError:\n            continue\n\n    if r_ref is None:\n        print(f"[{roi_name}] no valid pairs → SKIPPING PCF")\n        return\n\n    # compute PCFs for each original pair\n    pcf = {}\n    for (Ai, Ci), (Aj, Cj) in pairs:\n        key = f"{Ci} ⟷ {Cj}"\n        try:\n            _, g = ms.spatial_statistics.cross_pair_correlation_function(\n                domain, (Ai, Ci), (Aj, Cj),\n                max_R=250, annulus_step=5, annulus_width=10,\n                exclude_zero=True, boundary_exclude_distance=10,\n                visualise_output=False\n            )\n        except ValueError:\n            g = np.full_like(r_ref, np.nan)\n        pcf[key] = g\n\n    # save PCF CSV\n    os.makedirs(pcf_out_dir, exist_ok=True)\n    out_df = pd.DataFrame(pcf, index=r_ref)\n    out_df.index.name = \'R\'\n    out_path = os.path.join(pcf_out_dir, f"{roi_name}_pcf.csv")\n    out_df.to_csv(out_path)\n    print(f"[{roi_name}] PCF → {out_path}")\n\n\ndef process_slide(slide, input_root, pcf_root, scatter_root, n_workers, alpha):\n    in_dir = os.path.join(input_root, slide)\n    pcf_out_dir = os.path.join(pcf_root, slide)\n    scatter_out_dir = os.path.join(scatter_root, slide)\n\n    # list all ROI CSVs and extract numeric suffix\n    roi_files = sorted(\n        glob.glob(os.path.join(in_dir, "ROI_*.csv")),\n        key=lambda fn: int(os.path.splitext(os.path.basename(fn))[0].split("_")[-1])\n    )\n\n    # keep only those in the allowed list\n    roi_files = [fp for fp in roi_files\n                 if int(os.path.splitext(os.path.basename(fp))[0].split("_")[-1]) in ALLOWED_ROIS]\n\n    if not roi_files:\n        print(f"--- [{slide}] 0 specified ROIs → SKIPPING SLIDE ---")\n        return\n\n    print(f"--- [{slide}] processing {len(roi_files)} specified ROIs (parallel={n_workers}, alpha={alpha}) ---")\n    tasks = [(fp, pcf_out_dir, scatter_out_dir, alpha) for fp in roi_files]\n\n    with Pool(processes=min(n_workers, len(tasks))) as pool:\n        pool.map(process_roi, tasks)\n\n\ndef main():\n    parser = argparse.ArgumentParser(\n        description="Compute PCFs and scatter plots for a specified list of ROIs in parallel within each slide."\n    )\n    parser.add_argument(\n        "-i", "--input-root", required=True,\n        help="Root folder of condition (e.g. .../Spatial_Transcriptomics_data/Tumour)"\n    )\n    parser.add_argument(\n        "-p", "--pcf-root", required=True,\n        help="Where to write PCF CSVs (e.g. .../Spatial_Transcriptomics_data/PCF/Tumour)"\n    )\n    parser.add_argument(\n        "-s", "--scatter-root", required=True,\n        help="Where to write scatter plots (e.g. .../Spatial_Transcriptomics_data/Scattered/Tumour)"\n    )\n    parser.add_argument(\n        "-n", "--processes", type=int, default=6,\n        help="Number of parallel ROI-level processes per slide (default: 6)"\n    )\n    parser.add_argument(\n        "--alpha", type=float, default=95,\n        help="Alpha parameter for boundary estimation (default: 95)"\n    )\n    args = parser.parse_args()\n\n    slides = sorted([\n        d for d in os.listdir(args.input_root)\n        if os.path.isdir(os.path.join(args.input_root, d))\n    ])\n\n    for slide in slides:\n        process_slide(\n            slide, args.input_root, args.pcf_root,\n            args.scatter_root, args.processes,\n            args.alpha\n        )\n\n    print("ALL SLIDES DONE")\n\nif __name__ == "__main__":\n    main()\n')


# In[ ]:


python process_missing_tans_rois_pcf.py \
  -i /Volumes/My\ Passport/Spatial_Transcriptomics_data_final/Peritumour \
  -p /Volumes/My\ Passport/Spatial_Transcriptomics_data_final/PCF/Peritumour \
  -s /Volumes/My\ Passport/Spatial_Transcriptomics_data_final/Scattered/Peritumour \
  --alpha 120 \
...
# ── USER SETTINGS ──────────────────────────────────────────
region       = "Tumour"
input_root   = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/PCF"
output_root  = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Averaged_ROIs_Per_Region/PCF"
# ────────────────────────────────────────────────────────────
...
# ── USER SETTINGS ──────────────────────────────────────────
region         = "Tumour"
stats_csv      = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Averaged_ROIs_Per_Region/PCF/Tumour/batch_Tumour_pcf_stats.csv"
counts_csv     = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Averaged_ROIs_Per_Region/PCF/Tumour/batch_Tumour_pair_counts.csv"
...
# ── USER SETTINGS ──────────────────────────────────────────
region         = "Peritumour"
stats_csv      = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Averaged_ROIs_Per_Region/PCF/Peritumour/batch_Peritumour_pcf_stats.csv"
counts_csv     = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Averaged_ROIs_Per_Region/PCF/Peritumour/batch_Peritumour_pair_counts.csv"
save_plots_to  = os.path.join(os.path.dirname(stats_csv), "Individual_Pair_Plots")
os.makedirs(save_plots_to, exist_ok=True)
...
# ── USER SETTINGS ──────────────────────────────────────────
region         = "Healthy"
stats_csv      = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Averaged_ROIs_Per_Region/PCF/Healthy/batch_Healthy_pcf_stats.csv"
counts_csv     = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Averaged_ROIs_Per_Region/PCF/Healthy/batch_Healthy_pair_counts.csv"
save_plots_to  = os.path.join(os.path.dirname(stats_csv), "Individual_Pair_Plots")
os.makedirs(save_plots_to, exist_ok=True)
...
input_root = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/PCF"
exclude_dir = "/Volumes/My Passport/Spatial_Transcriptomics_data_final"

# OUTPUT: new results folder for this exclusion
main_out_root = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Averaged_ROIs_Per_Region"
