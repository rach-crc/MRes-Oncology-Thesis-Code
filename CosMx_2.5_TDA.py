...
# Volcano plots with Top-20 highlights and legends including non-significant entries
import os
import sys
import pandas as pd
import numpy as np
...
TOP20_DIR   = os.path.join(ROOT, "TDA_NC", "rf_top20_features_lists")  # Top20_{r1}_vs_{r2}.csv
OUTDIR      = os.path.join(ROOT, "TDA_NC", "volcano_plots_TDA_top20_highlighted_with_nonsig")
...
for r1, r2 in REGION_PAIRS:
    comp = f"{r1}_vs_{r2}"
    top20_path = os.path.join(TOP20_DIR, f"Top20_{comp}.csv")
    highlights = load_highlight_features(top20_path) if os.path.isfile(top20_path) else []

    # map highlighted features to distinct colors
    highlight_colors = {f: PALETTE[i % len(PALETTE)] for i, f in enumerate(highlights)}

    fout = os.path.join(OUTDIR, f"volcano_{comp}_top20highlight_with_nonsig.png")
    ...
    for f in all_feats:
        ...
        # highlight if in Top20 list; otherwise grey
    ...
    ax.set_title(f"Volcano: {r1} vs {r2} (Top-20 highlighted; non-sig labeled)", fontsize=16)
    ...
    if pos_all:
        ax.legend(pos_handles, pos_all, title=r2,
                  loc='upper left', bbox_to_anchor=(1.7, 1),
                  frameon=False, fontsize=9, title_fontsize=11)
...
# ---- CONFIG ----
ROOT = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/TDA_NC"
OUT_DIR = os.path.join(ROOT, "rf_top20_features_lists")
os.makedirs(OUT_DIR, exist_ok=True)
...
for region1, region2 in REGION_PAIRS:
    ...
    top20.insert(1, "comparison", comp_tag)

    # Save CSV for this comparison
    out_csv = os.path.join(OUT_DIR, f"Top20_{comp_tag}.csv")
    top20.to_csv(out_csv)
    print(f"\n=== {comp_tag} ===")
    print(top20[["rank", "comparison", "mean_importance"]])

    # Add to combined list
    top20_reset = top20.reset_index().rename(columns={"index": "feature"})
    all_top20_records.append(top20_reset)

# Save combined table
if all_top20_records:
    combined = pd.concat(all_top20_records, ignore_index=True)
    combined_out = os.path.join(OUT_DIR, "Top20_features_all_comparisons.csv")
    combined.to_csv(combined_out, index=False)
    print(f"\nSaved combined top-20 table: {combined_out}")
...
# ---- CONFIG ----
ROOT = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/TDA_NC"
COMBINED_CSV = os.path.join(ROOT, "rf_top20_features_lists", "Top20_features_all_comparisons.csv")

# ---- Load Combined Table ----
df = pd.read_csv(COMBINED_CSV)

# ---- Get union of all top-20 features across all comparisons ----
unique_features = sorted(df["feature"].unique())

# ---- Print the list and the count ----
print("\n=== Unique features in any Top-20 (union across all comparisons) ===")
for f in unique_features:
    print(f)
print(f"\nTotal: {len(unique_features)} unique features appeared in the top-20 for any comparison.")
...
# --- CONFIG ---
PLOT_DIR = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/TDA_NC/boxplots_per_feature"
OUT_PDF  = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/TDA_NC/combined_top_features_boxplots_A4.pdf"
...
features = [
    ...
    "Cluster_0⟷Cluster_7_H1_significant_lifespan_std",
    "Cluster_0⟷Cluster_7_H1_significant_nBars",
    "Cluster_2_H0_significant_lifespan_mean",
    "Cluster_2_H0_significant_lifespan_std",
    "Cluster_2_H0_significant_nBars",
    "Cluster_2_H1_significant_birth_mean",
    "Cluster_2_H1_significant_lifespan_mean",
    "Cluster_2_H1_significant_nBars",
    "Cluster_2⟷Cluster_7_H0_significant_lifespan_mean",
    "Cluster_2⟷Cluster_7_H0_significant_lifespan_std",
    "Cluster_2⟷Cluster_7_H0_significant_nBars",
    "Cluster_2⟷Cluster_7_H1_significant_birth_mean",
    "Cluster_2⟷Cluster_7_H1_significant_birth_std",
    "Cluster_2⟷Cluster_7_H1_significant_lifespan_mean",
    "Cluster_2⟷Cluster_7_H1_significant_lifespan_std",
    "Cluster_2⟷Cluster_7_H1_significant_nBars",
...
with PdfPages(OUT_PDF) as pdf:
    for page_start in range(0, len(features), plots_per_page):
        ...
        fig.subplots_adjust(
            hspace=0.12,   # try 0.10–0.20 for more/less space
            wspace=0.04,
            left=0.04,
            right=0.96,
            top=0.97,
        ...
        pdf.savefig(fig, dpi=350)  # High resolution, no bbox_inches!
        plt.close(fig)
