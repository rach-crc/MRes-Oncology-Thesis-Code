...
# Done: Peritumour, Tumour, Healthy
...
# [Peritumour] Saved cross-ROI averages for 513 ROIs → '/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_cell_count/Peritumour/Peritumour_average_normalized_counts.csv'


# In[ ]:


# [Tumour] Saved cross-ROI averages for 572 ROIs → '/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_cell_count/Tumour/Tumour_average_normalized_counts.csv'


# In[ ]:


# [Healthy] Saved cross-ROI averages for 532 ROIs → '/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_cell_count/Healthy/Healthy_average_normalized_counts.csv'
...
BASE_DIR = "/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_cell_count"
...
color_map = {
    "FAP+ CAFs": "#b50404",
    "aSMA+ CAFs": "#eb1c1c",
    "aSMA+FAP+ CAFs": "#fc4444",
    "lymphatic CAFs": "#f57171",
    "Endothelial cells": "#f57c16",
    "Hepatocytes": "#f5f50a",
    "Neutrophils": "#518508",
    "Macrophages": "#a0db4d",
    "Tumour cells": "#f25aba",
    "proliferating Tumour cells": "#fa96d5",
    "CD8 T cells": "#1b5f94",
    "exhausted CD8 T cells 1": "#217dc4",
    "exhausted CD8 T cells 2": "#3fa0eb",
    "proliferating CD8 T cells": "#72bdf7",
    "CD4 T cells": "#540f61",
    "exhausted CD4 T cells 1": "#81368f",
    "exhausted CD4 T cells 2": "#b384bd",
    "proliferating CD4 T cells": "#a006bd",
    "T-reg": "#e897f7",
    "T cells": "#e3cfe8",
    "Other Immune cells": "#51989e",
    "Non-immune cells": "#92e8f0",
    "Unknown": "#f5edc6"
}
...
# [Healthy] Combined 532 ROI files → saved '/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_cell_count/Healthy/Healthy_all_ROIs_normalized_counts.csv'.
# [Healthy] Number of columns (excluding ['Region', 'Slide', 'ROI']): 23

# [Peritumour] Combined 513 ROI files → saved '/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_cell_count/Peritumour/Peritumour_all_ROIs_normalized_counts.csv'.
# [Peritumour] Number of columns (excluding ['Region', 'Slide', 'ROI']): 23

# [Tumour] Combined 572 ROI files → saved '/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_cell_count/Tumour/Tumour_all_ROIs_normalized_counts.csv'.
# [Tumour] Number of columns (excluding ['Region', 'Slide', 'ROI']): 23
...
# ────────────── EDIT THIS PATH ──────────────
ROOT_DIR = "/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_cell_count"
# ────────────────────────────────────────────
...
for region in sorted(agg["Region"].unique()):
    ...
    ax.set_xticklabels(df_region.index, rotation=90, ha="center", fontsize=11)
...
TARGET_CELL_TYPES = [
    ...
    "exhausted CD8 T cells 2","proliferating CD4 T cells","T cells",
    "lymphatic CAFs","Other Immune cells","Endothelial cells"
]
...
TARGET_CELL_TYPES = [
    "FAP+ CAFs","aSMA+ CAFs","aSMA+FAP+ CAFs",
    "Macrophages","Neutrophils","exhausted CD4 T cells 2",
    "CD8 T cells","exhausted CD8 T cells 1","proliferating CD8 T cells",
    "CD4 T cells","T-reg","exhausted CD4 T cells 1",
    "exhausted CD8 T cells 2","proliferating CD4 T cells",
    "T cells","lymphatic CAFs", "Other Immune cells","Endothelial cells"
]
...
PALETTE = [
    ...
    "#46f0f0","#1e26fc","#bcf60c","#fac8e7","#008080","#e6beff",
    "#9a6324","#70360f","#f04d63","#aaffc3","#808000","#ffd8b1",
    "#130a7a","#808080","#000000","#75dbfa","#2f5aa1","#b3aa70",
...
#    (expects columns: Slide, Region, <cell1>, <cell2>, …)
...
for r1, r2 in REGION_PAIRS:
    ...
    # 6) split & sort by sign of Δ
    delta_series = sub[['cell_type','delta']] \
                    .drop_duplicates() \
                    .set_index('cell_type')['delta']

    # negative group: most negative → least negative
    neg_items = [(ct, delta_series[ct]) for ct in delta_series.index if delta_series[ct] < 0]
    neg_items.sort(key=lambda x: x[1])          
    neg_cts = [ct for ct, _ in neg_items]

    # positive group: smallest positive → largest positive
    pos_items = [(ct, delta_series[ct]) for ct in delta_series.index if delta_series[ct] >= 0]
    ...
    # 7) Save volcano plot
    out_png = os.path.join(OUTDIR, f"volcano_{r1}_vs_{r2}.png")
    fig.savefig(out_png, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
# 2.1.2 Normalise cell count per ROI in ROIs with High Immune cell or CAF Infiltration
...
TARGET_CELL_TYPES = [
    ...
    "Other Immune cells", "Endothelial cells"
]
BROAD_TYPES = ["Immune Cells", "CAFs"]
# ────────────────────────────────────────────────────────────────────────────────

# ─── 2) LOAD EXCLUSION SETS ────────────────────────────────────────────────────
# exclude[region] = set of (broad_type, Slide, ROI)
exclude = {}
for reg in REGIONS:
    fn = os.path.join(EXCL_DIR, f"Exclude_{reg}_rois.csv")
    if os.path.exists(fn):
        df_ex = pd.read_csv(fn, dtype=str)
...
for region in REGIONS:
    ...
    for fp in glob.glob(pattern):
        ...
        for ct in TARGET_CELL_TYPES:
            row[ct] = float(df_roi[ct].iloc[0]) if ct in df_roi.columns else 0.0
        records.append(row)
...
# ─── 5) LOOP OVER BROAD TYPES ─────────────────────────────────────────────────
for bt in BROAD_TYPES:
    print(f"\n===== ANALYSIS FOR {bt} =====")

    # 5a) Build exclusion sets per region (for this broad type)
    exclude_bt = {}
    for reg in REGIONS:
        # all (Slide,ROI) pairs to EXCLUDE for this bt in this region:
        exclude_bt[reg] = {
            (s, r) for (b, s, r) in exclude[reg] if b == bt
        }
        print(f"{bt} in {reg}: {len(exclude_bt[reg])} ROIs excluded")

    # 5b) Filter df_long *out* those ROIs
    mask = df_long.apply(
        lambda r: (r["Slide"], r["ROI"]) not in exclude_bt[r["Region"]],
        axis=1
    )
    df_filt = df_long[mask]
    if df_filt.empty:
        print(f"→ No data left for {bt}, skipping.")
        continue

    # 5c) Compute slide-level means
    slide_means_long = (
        df_filt
        .groupby(["Slide","Region","cell_type"], as_index=False)["frac"]
        .mean()
    ...
    out_slide = os.path.join(
        ROOT_DIR,
        f"slide_means_normalized_counts_{bt.replace(' ','_')}.csv"
    )
    slide_means.to_csv(out_slide, index=False)
    print(f"Saved → {out_slide}")

    # 5d) Paired Wilcoxon + BH–FDR
    results = []
    for r1, r2 in PAIRS:
        df1 = slide_means[slide_means.Region==r1].set_index("Slide")
        df2 = slide_means[slide_means.Region==r2].set_index("Slide")
    ...
    for r1, r2 in PAIRS:
        sub = pd.DataFrame(
            [r for r in results if r["Region1"]==r1 and r["Region2"]==r2]
        )
        pvals = sub["p_value"].fillna(1.0).values
        ...
        sub["reject_fdr"] = reject
        final.append(sub)
    paired_df = pd.concat(final, ignore_index=True)

    out_paired = os.path.join(
        ROOT_DIR,
        f"paired_tests_targeted_FDR_{bt.replace(' ','_')}.csv"
    )
...
COLORS         = {"Healthy":"lightgreen","Peritumour":"skyblue","Tumour":"orange"}
OUT_BASE       = os.path.join(BASE_DIR, "across_region_boxplots_targeted")
# ────────────────────────────────────────────────────────────────────────────────

for bt in BROAD_TYPES:
    # set up filenames
    slide_means_fn = os.path.join(
        BASE_DIR,
        f"slide_means_normalized_counts_{bt}.csv"
    )
    fdr_fn = os.path.join(
        BASE_DIR,
        f"paired_tests_targeted_FDR_{bt}.csv"
    )
    # load
    df_means = pd.read_csv(slide_means_fn)
    fdr      = pd.read_csv(fdr_fn)

    # output dir per broad type
    out_dir = os.path.join(OUT_BASE, bt)
    os.makedirs(out_dir, exist_ok=True)

    # which column holds slide IDs?
    if "Slide" in df_means.columns:
        slide_col = "Slide"
    elif "slide_ID" in df_means.columns:
        slide_col = "slide_ID"
    else:
        raise KeyError("Neither 'Slide' nor 'slide_ID' found in slide_means")

    print(f"\n--- Generating boxplots for {bt} ---")

    #  for each cell_type
    for cell in TARGET_CELL_TYPES:
        # 1) assemble per-region data
        region_dfs = []
        for r in REGIONS:
            tmp = df_means[df_means.Region == r][[slide_col, cell]].dropna(subset=[cell]).copy()
            tmp[slide_col] = tmp[slide_col].astype(str).str.replace(r"^Slide_", "", regex=True)
            region_dfs.append(tmp)

        # 2) data arrays
        data = [df[cell].values for df in region_dfs]
        ...
        # 5) overlay points + annotate outliers
        for i, df_r in enumerate(region_dfs):
            arr       = df_r[cell].values
            slides    = df_r[slide_col].values
            lb, ub    = bounds[i]
            jitter    = np.random.normal(0, 0.03, size=len(arr)) if len(arr) else []
            for j,(val,sid) in enumerate(zip(arr,slides)):
                xj = i + jitter[j]
                ax.scatter(xj, val, color='k', s=9, alpha=0.7, zorder=3)
                if not np.isnan(lb) and not np.isnan(ub) and (val<lb or val>ub):
                    ax.text(xj, val, sid,
...
# ─── 1) CONFIG ───────────────────────────────────────────────────────────────
ROOT_DIR    = "/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_by_broad_type"
EXCL_DIR    = os.path.abspath(os.path.join(ROOT_DIR, os.pardir))
REGIONS     = ["Healthy", "Peritumour", "Tumour"]
PAIRS       = [
    ("Healthy", "Peritumour"),
...
TARGET_CELL_TYPES = [
    ...
    "proliferating CD8 T cells","CD4 T cells","T-reg","exhausted CD4 T cells 1",
    "exhausted CD8 T cells 2","proliferating CD4 T cells","T cells",
    "lymphatic CAFs","Other Immune cells"
]
BROAD_TYPES = ["Immune Cells", "CAFs"]
OUT_SLIDE   = os.path.join(ROOT_DIR, "slide_means_normalized_counts.csv")
OUT_PAIRED  = os.path.join(ROOT_DIR, "paired_tests_targeted_FDR.csv")
...
broad_map = {}
for ct in ["FAP+ CAFs","aSMA+ CAFs","aSMA+FAP+ CAFs","lymphatic CAFs"]:
    broad_map[ct] = "CAFs"
for ct in [
    "Macrophages","Neutrophils","exhausted CD4 T cells 2","CD8 T cells",
    "exhausted CD8 T cells 1","proliferating CD8 T cells","CD4 T cells","T-reg",
    "exhausted CD4 T cells 1","exhausted CD8 T cells 2","proliferating CD4 T cells",
...
print(f"{'Region':>10s}  {'Total':>7s}   {'Immune Cells':>13s}   {'CAFs':>5s}")
for reg in REGIONS:
    ...
    print(f"→ {reg:10s}  {total:7d}   {n_imm:13d}   {n_caf:5d}")
print()
...
# ─── CONFIG ────────────────────────────────────────────────────────────────
BASE_DIR       = "/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_by_broad_type"
SLIDE_MEANS    = os.path.join(BASE_DIR, "slide_means_normalized_counts.csv")
FDR_CSV        = os.path.join(BASE_DIR, "paired_tests_targeted_FDR.csv")
REGIONS        = ["Healthy","Peritumour","Tumour"]
PAIRS          = [
...
TARGET_CELL_TYPES = [
    ...
    "CD4 T cells","T-reg","exhausted CD4 T cells 1",
    "exhausted CD8 T cells 2","proliferating CD4 T cells",
    "T cells","lymphatic CAFs", "Other Immune cells"
]
OUTPUT_DIR     = os.path.join(BASE_DIR, "across_region_boxplots_targeted")
COLORS         = {"Healthy":"lightgreen","Peritumour":"skyblue","Tumour":"orange"}
# ────────────────────────────────────────────────────────────────────────────
...
for r1, r2 in REGION_PAIRS:
    ...
    out = os.path.join(OUTDIR, f"volcano_{r1.replace(' ','_')}_vs_{r2.replace(' ','_')}.png")
...
broad_type_map = {
    ...
    "exhausted CD8 T cells 1": "Immune Cells",
    "exhausted CD8 T cells 2": "Immune Cells",
    "proliferating CD8 T cells": "Immune Cells",
    "T-reg": "Immune Cells",
    "T cells": "Immune Cells",
    "Other Immune cells": "Immune Cells",
    "CD4 T cells": "Immune Cells",
    "exhausted CD4 T cells 1": "Immune Cells",
    "exhausted CD4 T cells 2": "Immune Cells",
    "proliferating CD4 T cells": "Immune Cells",
...
get_ipython().run_cell_magic('writefile', 'Combine0607_broad_cell_type_region_level_counts.py', 'import os\nimport glob\nimport pandas as pd\n\n# ─── EDIT ONLY THIS ───────────────────────────────────────────────────\n# Base folder containing per‐ROI normalized counts:\nROOT_NORM = "/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_by_broad_type"\n# ──────────────────────────────────────────────────────────────────────\n\ndef combine_region(region_dir):\n    """\n    For a given region folder, find all ROI_*_by_broadtype.csv under any Slide_* subfolder,\n    concatenate them into a single DataFrame, save that CSV in the region folder,\n    and print the number of columns excluding the three metadata columns.\n    """\n    region_name = os.path.basename(region_dir.rstrip("/"))\n    all_csvs = glob.glob(os.path.join(region_dir, "Slide_*", "*", "*_by_broadtype.csv"), recursive=False)\n\n    if not all_csvs:\n        print(f"[{region_name}] No ROI CSV files found under {region_dir!r}.")\n        return\n\n    df_list = []\n    for fp in sorted(all_csvs):\n        df = pd.read_csv(fp)\n        df_list.append(df)\n\n    # Concatenate all per‐ROI DataFrames (they share the same columns)\n    combined = pd.concat(df_list, axis=0, ignore_index=True, sort=False).fillna(0)\n\n    # Save combined CSV into the region folder\n    out_csv = os.path.join(region_dir, f"{region_name}_all_ROIs_normalized_counts.csv")\n    combined.to_csv(out_csv, index=False)\n\n    # Count columns excluding the three metadata columns: Region, Slide, ROI\n    total_cols = combined.shape[1]\n    metadata_cols = [c for c in ("Region", "Slide", "ROI") if c in combined.columns]\n    num_data_cols = total_cols - len(metadata_cols)\n\n    print(f"[{region_name}] Combined {len(df_list)} ROI files → saved \'{out_csv}\'.")\n    print(f"[{region_name}] Number of columns (excluding {metadata_cols}): {num_data_cols}\\n")\n\n\ndef main():\n    if not os.path.isdir(ROOT_NORM):\n        raise FileNotFoundError(f"Root folder not found: {ROOT_NORM!r}")\n\n    # List all immediate subdirectories (each should be a region)\n    regions = [\n        os.path.join(ROOT_NORM, d) for d in os.listdir(ROOT_NORM)\n        if os.path.isdir(os.path.join(ROOT_NORM, d))\n    ]\n    if not regions:\n        print(f"No region folders found under {ROOT_NORM!r}.")\n        return\n\n    for region_dir in sorted(regions):\n        combine_region(region_dir)\n\n\nif __name__ == "__main__":\n    main()\n')


# In[ ]:


# [Healthy] Combined 532 ROI files → saved '/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_by_broad_type/Healthy/Healthy_all_ROIs_normalized_counts.csv'.
# [Healthy] Number of columns (excluding ['Region', 'Slide', 'ROI']): 17

# [Peritumour] Combined 513 ROI files → saved '/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_by_broad_type/Peritumour/Peritumour_all_ROIs_normalized_counts.csv'.
# [Peritumour] Number of columns (excluding ['Region', 'Slide', 'ROI']): 17

# [Tumour] Combined 572 ROI files → saved '/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_by_broad_type/Tumour/Tumour_all_ROIs_normalized_counts.csv'.
# [Tumour] Number of columns (excluding ['Region', 'Slide', 'ROI']): 17
...
# ─── CONFIGURATION ─────────────────────────────────────────────────────────────
ROOT       = "/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_cell_count"
...
plt.figure(figsize=(7,6))
for slide in slides:
    m = coords_df.index.get_level_values("Slide")==slide
    plt.scatter(coords[m,0], coords[m,1],
                label=slide, s=12, alpha=0.7, color=slide_colors[slide])
...
fig, ax = plt.subplots(figsize=(7,6))
for slide in slides:
    m = umap_df.index.get_level_values("Slide")==slide
    ax.scatter(Z[m,0], Z[m,1],
               label=slide, s=12, alpha=0.7,
               color=slide_colors[slide], edgecolors="none")
...
# Map cell types to broad groups for ROI exclusion logic
broad_map = {}
for ct in ["FAP+ CAFs", "aSMA+ CAFs", "aSMA+FAP+ CAFs", "lymphatic CAFs"]:
    broad_map[ct] = "CAFs"
for ct in [
    "Neutrophils","exhausted CD4 T cells 2","Macrophages","CD8 T cells",
    "exhausted CD8 T cells 1","proliferating CD8 T cells","CD4 T cells","T-reg",
    "exhausted CD4 T cells 1","exhausted CD8 T cells 2","proliferating CD4 T cells",
    "T cells","Other Immune cells"
]:
    broad_map[ct] = "Immune Cells"
# Note: Endothelial cells are features only, not used for grouping

ALL_CELL_TYPES = TARGET_CELL_TYPES
BROAD_TYPES     = sorted(set(broad_map.values()))  # ['CAFs', 'Immune Cells']

# Exclusion files directory
EXCL_ROOT = os.path.dirname(ROOT)

# Output directory
tmp = "BroadType_PCA_UMAP_with_ComBat_and_ROI_filter"
OUT_BASE = os.path.join(ROOT, tmp)
...
def load_and_filter_data():
    """
    Load all regions with all TARGET_CELL_TYPES features.
    """
    dfs = []
    for region in REGIONS:
        path = os.path.join(ROOT, region, SUMMARY_FN.format(region=region))
        df   = pd.read_csv(path).fillna(0.0)
        df   = df[INPUT_META + TARGET_CELL_TYPES].copy()
        df['Region'] = region
        dfs.append(df)
    big_df = pd.concat(dfs, ignore_index=True)
...
def apply_exclusions(big_df, region, broad_type):
    ...
    mask      = pd.Series(True, index=big_df.index)
    excl_set  = set()
    if os.path.isfile(excl_path):
        ...
        if not excl.empty:
            ...
            print(f"{broad_type}, {region}: excluding {len(excl_set)} ROIs")
            mask = ~big_df.apply(
        ...
        print(f"Warning: No exclusion file for {region}, keeping all ROIs.")
    return mask, excl_set

# ─── MAIN ANALYSIS ─────────────────────────────────────────────────────────────
# Load full dataset
full_df = load_and_filter_data()
...
for broad_type in BROAD_TYPES:
    ...
    # Apply exclusions based on this broad_type
    masks = []
    all_excl = set()
    for region in REGIONS:
        m, excl = apply_exclusions(full_df, region, broad_type)
        masks.append(m)
        all_excl.update(excl)
    final_mask = np.logical_and.reduce(masks)
    filtered = full_df[final_mask].copy()
    print(f"Total ROIs excluded for {broad_type}: {len(all_excl)}")

    # Save tables
    full_df.to_csv(os.path.join(out_dir, 'all_regions_unfiltered_counts.csv'), index=False)
    ...
    feats = TARGET_CELL_TYPES

    # ComBat batch correction
    adata = sc.AnnData(filtered[feats].values)
    adata.obs['batch']  = filtered.index.get_level_values('Slide')
    adata.obs['region'] = filtered.index.get_level_values('Region')
    sc.pp.combat(adata, key='batch', covariates=['region'])
    Xs = StandardScaler().fit_transform(adata.X)
    ...
    plt.figure(figsize=(7,6))
    for r in REGIONS:
        mask_r = coords_df.index.get_level_values('Region') == r
        plt.scatter(coords[mask_r,0], coords[mask_r,1], label=r,
                    color=color_map[r], s=10, alpha=0.7)
    plt.legend(bbox_to_anchor=(1.02,1), loc='upper left', frameon=False, title='Region')
    plt.xlabel('PC1'); plt.ylabel('PC2')
    plt.title(f'PCA ({broad_type})')
    plt.tight_layout(rect=[0.8,0.8,1,1])
    plt.savefig(os.path.join(out_dir, 'PCA_by_region.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.figure(figsize=(7,6))
    for s in slides:
        mask_s = coords_df.index.get_level_values('Slide') == s
        plt.scatter(coords[mask_s,0], coords[mask_s,1], label=s,
                    color=slide_colors[s], s=10, alpha=0.7)
    plt.legend(bbox_to_anchor=(1.02,1), loc='upper left', frameon=False,
               title='Slide', fontsize='small')
    plt.xlabel('PC1'); plt.ylabel('PC2')
    plt.title(f'PCA ({broad_type} by Slide)')
    plt.tight_layout(rect=[0.8,0.8,1,1])
    plt.savefig(os.path.join(out_dir, 'PCA_by_slide.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    fig, ax = plt.subplots(figsize=(7,6))
    for r in REGIONS:
        mask_r = umap_df.index.get_level_values('Region') == r
        ax.scatter(Z[mask_r,0], Z[mask_r,1], label=r,
                   color=color_map[r], s=12, alpha=0.7, edgecolors='none')
    ax.legend(bbox_to_anchor=(1.02,1), loc='upper left', frameon=False, title='Region')
    ax.set_xlabel('UMAP1'); ax.set_ylabel('UMAP2')
    ax.set_title(f'UMAP ({broad_type})')
    plt.tight_layout(rect=[0.8,0.8,1,1])
    fig.savefig(os.path.join(out_dir, 'UMAP_by_region.png'), dpi=300, bbox_inches='tight')
    plt.close(fig)

    # UMAP by Slide
    fig, ax = plt.subplots(figsize=(7,6))
    for s in slides:
        mask_s = umap_df.index.get_level_values('Slide') == s
        ax.scatter(Z[mask_s,0], Z[mask_s,1], label=s,
                   color=slide_colors[s], s=12, alpha=0.7, edgecolors='none')
    ax.legend(bbox_to_anchor=(1.02,1), loc='upper left', frameon=False,
              title='Slide', fontsize='small')
    ax.set_xlabel('UMAP1'); ax.set_ylabel('UMAP2')
    ax.set_title(f'UMAP ({broad_type} by Slide)')
    plt.tight_layout(rect=[0.8,0.8,1,1])
    fig.savefig(os.path.join(out_dir, 'UMAP_by_slide.png'), dpi=300, bbox_inches='tight')
    plt.close(fig)

print("\nAll broad-type analyses complete!")
...
# ==== CONFIGURATION ====
ROOT = "/Volumes/My Passport/Spatial_Proteomics_data_final/Normalized_by_broad_type"
REGIONS = ["Tumour", "Peritumour", "Healthy"]
SUMMARY_FN = "{region}_all_ROIs_normalized_counts.csv"
...
for broad_type in BROAD_TYPES:
    ...
    filtered.set_index(["Region", "Slide", "ROI"], inplace=True)
    # --- ComBat (remove slide effect) ---
    adata = sc.AnnData(filtered[feats].values)
    adata.obs["batch"]  = filtered.index.get_level_values("Slide")
    adata.obs["region"] = filtered.index.get_level_values("Region")
    # 4) ComBat (preserving region differences)
    sc.pp.combat(adata, key="batch", covariates=["region"])
    X_corr = adata.X
    # --- Standardize ---
    Xs = StandardScaler().fit_transform(X_corr)
