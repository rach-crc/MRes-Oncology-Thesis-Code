...
# Filtered ROIs must contain any of 0 2 7
...
out_paired     = os.path.join(
    neigh_dir,
    "paired_wilcoxon_fdr_target_clusters_filteredROIs.csv"
)

# ─── 2) LOAD CSVs AND FILTER ROIs ─────────────────────────────────────────────
csv_paths = glob.glob(os.path.join(neigh_dir, "*_ROI_cluster_proportions.csv"))
# load all
df_all    = pd.concat((pd.read_csv(p) for p in csv_paths), ignore_index=True)
# keep only ROIs with any target cluster > 0
mask_roi  = (df_all[TARGET_COLS] > 0).any(axis=1)
filtered  = df_all[mask_roi].reset_index(drop=True)
print(f"Filtered {len(filtered)}/{len(df_all)} total ROIs with any of clusters {TARGET_CLUSTERS}")
...
# ─── 5) SET UP REGION PAIRS AND CLUSTERS ─────────────────────────────────────
regions = sorted(slide_means_long["Region"].unique())
PAIRS   = list(combinations(regions, 2))
clusters = TARGET_COLS  # only targeted clusters
...
for r1, r2 in PAIRS:
    ...
    df1 = (
        slide_means_long[slide_means_long.Region == r1]
        .pivot(index="Slide", columns="Cluster", values="mean_frac")
    )
    df2 = (
        slide_means_long[slide_means_long.Region == r2]
        .pivot(index="Slide", columns="Cluster", values="mean_frac")
    )
    common = df1.index.intersection(df2.index)

    # Count total ROIs for each region (not per-cluster)
    total_rois_r1 = filtered[filtered.Region == r1]["ROI"].nunique()
    total_rois_r2 = filtered[filtered.Region == r2]["ROI"].nunique()
...
# ─── 7) FDR correction per region‑pair ────────────────────────────────────────
df_res     = pd.DataFrame(results)
out_frames = []
for (r1, r2), grp in df_res.groupby(["Region1", "Region2"]):
    pvals           = grp["p_value"].fillna(1.0).values
    reject, p_adj, _, _ = multipletests(pvals, alpha=0.05, method="fdr_bh")
    grp = grp.assign(p_adj_fdr=p_adj, reject_fdr=reject)
...
# ─── CONFIG ────────────────────────────────────────────────────────────────
NEIGH_DIR   = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Neighbourhoods"
FDR_CSV     = os.path.join(NEIGH_DIR, "paired_wilcoxon_fdr_target_clusters_filteredROIs.csv")
OUTPUT_DIR  = os.path.join(NEIGH_DIR, "across_region_boxplots_clusters_filteredROIs")
REGIONS     = ["Healthy", "Peritumour", "Tumour"]
PAIRS       = [
    ("Healthy", "Peritumour"),
    ("Healthy", "Tumour"),
...
NEIGH_DIR    = "/Volumes/My Passport/Spatial_Transcriptomics_data_final/Neighbourhoods"
FDR_CSV      = os.path.join(NEIGH_DIR, "paired_wilcoxon_fdr_target_clusters_filteredROIs.csv")
OUTDIR       = os.path.join(NEIGH_DIR, "volcano_plots_clusters_filteredROIs")
REGION_PAIRS = [
    ("Healthy",   "Peritumour"),
    ("Healthy",   "Tumour"),
    ("Peritumour","Tumour"),
...
OUT_BASE   = os.path.join(ROOT, "Targeted_PCA_UMAP_filtered_RAW_combat")
...
for region in REGIONS:
    ...
    # FILTER: Only ROIs with any target cluster present (any >0)
    mask = (df_sub[TARGET_COLS].values > 0).any(axis=1)
