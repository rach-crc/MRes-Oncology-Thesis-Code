...
ROOT     = "/Volumes/My Passport/Spatial_Proteomics_data_final"
...
selected_clusters = {"Cluster_0", "Cluster_4", "Cluster_5", "Cluster_7"}
# pairs to consider: 0⟷4,0⟷5,0⟷7,4⟷7,5⟷7
pair_prefixes = [
    "Cluster_0⟷Cluster_4",
    "Cluster_0⟷Cluster_5",
    "Cluster_0⟷Cluster_7",
...
single_prefixes = [f"Cluster_{i}" for i in [0,4,5,7]]
...
# ---- CONFIG ----
ROOT = "/Volumes/My Passport/Spatial_Proteomics_data_final/TDA_NC"
REGIONS = ["Tumour", "Peritumour", "Healthy"]
# These are your single clusters; pairwise will be built from these
SINGLES = ["Cluster_0", "Cluster_4", "Cluster_5", "Cluster_7"]
# Output path (aggregate Spearman results)
OUT_CSV = os.path.join(ROOT, "all_regions_slide_0457stats_spearman_FDR.csv")
# ----------------
...
# --- CONFIG ---
INPUT_CSV = "/Volumes/My Passport/Spatial_Proteomics_data_final/TDA_NC/all_regions_slide_0457stats_spearman_FDR.csv"
OUT_PNG = "/Volumes/My Passport/Spatial_Proteomics_data_final/TDA_NC/correlations_heatmap_lower_triangle.png"
...
TARGET_CLUSTERS = ["Cluster_0","Cluster_4","Cluster_5","Cluster_7"]
# ----------------
...
# Only keep immune‐related features (clusters 0,4,5,7 or their pairs)
TARGET_CLUSTERS = {"Cluster_0", "Cluster_4", "Cluster_5", "Cluster_7"}
...
# Volcano plots with Top-30 highlights and legends including non-significant entries
import os
import sys
import pandas as pd
import numpy as np
...
TOP30_DIR   = os.path.join(ROOT, "TDA_NC", "rf_top30_features_lists")  # Top30_{r1}_vs_{r2}.csv
OUTDIR      = os.path.join(ROOT, "TDA_NC", "volcano_plots_TDA_top30_highlighted_with_nonsig")
...
# Only immune clusters (0,4,5,7) or pairs among them
IMMUNE_CLUSTERS = {"Cluster_0","Cluster_4","Cluster_5","Cluster_7"}

def is_immune_feature(feat: str) -> bool:
...
def load_highlight_features(path: str) -> list:
    df = pd.read_csv(path)
    ...
    if "Unnamed: 0" in df.columns:
        return df["Unnamed: 0"].astype(str).tolist()
    return df.index.astype(str).tolist()
...
# medians per Region × Feature (for delta on x-axis)
long = slides[['Slide','Region','Feature','Mean']]
med = (long.groupby(['Feature','Region'])['Mean'].median().reset_index())
...
PALETTE = [
    ...
    "#f58231", "#70360f", "#f04d63", "#aaffc3", "#ffd8b1", "#130a7a", "#e8be35", "#9dfcf8", 
...
def base_name(s: str) -> str:
    # strip " (non-significant)" etc. when needed
    return s.split(" (")[0]

# ─── PLOTTING ──────────────────────────────────────────────────────────
for r1, r2 in REGION_PAIRS:
    comp = f"{r1}_vs_{r2}"
    top30_path = os.path.join(TOP30_DIR, f"Top30_{comp}.csv")
    highlights = load_highlight_features(top30_path) if os.path.isfile(top30_path) else []

    # map highlighted features to distinct colors
    highlight_colors = {f: PALETTE[i % len(PALETTE)] for i, f in enumerate(highlights)}

    fout = os.path.join(OUTDIR, f"volcano_{comp}_top30highlight_with_nonsig.png")
    fig, ax = plt.subplots(figsize=(25, 12))

    # track lists (significant vs non-significant) for both sides
    neg_feats_sig, pos_feats_sig = [], []
    neg_feats_nonsig, pos_feats_nonsig = [], []
    deltas = {}

    for f in all_feats:
        if not is_immune_feature(f):
            continue
        if f not in med.index or r1 not in med.columns or r2 not in med.columns:
            continue

        v1, v2 = med.loc[f, r1], med.loc[f, r2]
        ...
        p_adj  = float(row.P_adj.values[0])
        signif = bool(row.Significant.values[0])
        d = v2 - v1
        neglogp = -np.log10(p_adj if p_adj > 0 else 1e-300)

        # highlight if in Top30 list; otherwise grey
        color = highlight_colors.get(f, GREY)
        alpha = 1.0 if f in highlight_colors else 0.5

        ax.scatter(
            d, neglogp,
            color=color,
            edgecolors='k' if f in highlight_colors else 'none',
            linewidths=0.6 if f in highlight_colors else 0.0,
            s=80 if f in highlight_colors else 40,
            zorder=3 if f in highlight_colors else 1,
            alpha=alpha
        )
        ...
        if f in highlight_colors:
            deltas[f] = d
            if d < 0:
                (neg_feats_sig if signif else neg_feats_nonsig).append(f if signif else f + " (non-significant)")
            else:
                (pos_feats_sig if signif else pos_feats_nonsig).append(f if signif else f + " (non-significant)")

    # horizontal FDR threshold
    ax.axhline(-np.log10(0.05), ls='--', c='k', lw=1)
    ax.set_xlabel(f"{r2} median – {r1} median", fontsize=14)
    ax.set_ylabel("-log10(FDR)", fontsize=14)
    ax.set_title(f"Volcano: {r1} vs {r2} (Top-30 highlighted; non-sig labeled)", fontsize=16)

    # order legend entries by delta; use base feature names for sorting/color
    neg_all = sorted(set(neg_feats_sig + neg_feats_nonsig), key=lambda x: deltas.get(base_name(x), 0))
    pos_all = sorted(set(pos_feats_sig + pos_feats_nonsig), key=lambda x: deltas.get(base_name(x), 0))

    neg_handles = [Line2D([0],[0], marker='o',
                          color=highlight_colors.get(base_name(f), GREY),
                          markeredgecolor='k', linestyle='', markersize=6)
                   for f in neg_all]
    pos_handles = [Line2D([0],[0], marker='o',
                          color=highlight_colors.get(base_name(f), GREY),
                          markeredgecolor='k', linestyle='', markersize=6)
                   for f in pos_all]
    ...
    if neg_all:
        leg1 = ax.legend(neg_handles, neg_all, title=r1,
                         loc='upper left', bbox_to_anchor=(1, 1),
                         frameon=False, fontsize=9, title_fontsize=11)
        ax.add_artist(leg1)
    if pos_all:
        ax.legend(pos_handles, pos_all, title=r2,
                  loc='upper left', bbox_to_anchor=(1.45, 1),
                  frameon=False, fontsize=9, title_fontsize=11)
...
OUT_DIR = os.path.join(ROOT, "0457_TDAs_PCA_UMAP_combat")
os.makedirs(OUT_DIR, exist_ok=True)
...
# ---- CONFIG ----
ROOT = "/Volumes/My Passport/Spatial_Proteomics_data_final/TDA_NC"
OUT_DIR = os.path.join(ROOT, "rf_top30featuregroup_PCA_UMAP_combat")
os.makedirs(OUT_DIR, exist_ok=True)
...
for region1, region2 in REGION_PAIRS:
    print(f"\n=== {region1} vs {region2} ===")
    # 1) Load RF importances & pick top 30
    ...
    plt.xlabel("PC1"); plt.ylabel("PC2")
    plt.title(f"PCA: {region1} vs {region2} [top30 RF features, ComBat]")
    plt.legend(bbox_to_anchor=(1.02,1), loc="upper left",
               frameon=False, title="Slide", fontsize=8)
    plt.tight_layout(rect=[0.8,0.8,1,1])
    plt.savefig(os.path.join(OUT_DIR, f"PCA30_ComBat_{region1}_vs_{region2}_bySlide.png"),
    ...
    plt.legend(bbox_to_anchor=(1.02,1), loc="upper left",
               frameon=False, title="Region")
    plt.tight_layout(rect=[0.8,0.8,1,1])
    plt.savefig(os.path.join(OUT_DIR, f"PCA30_ComBat_{region1}_vs_{region2}_byRegion.png"),
                dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.title(f"UMAP: {region1} vs {region2} [top30 RF features, ComBat]")
    plt.legend(bbox_to_anchor=(1.02,1), loc="upper left",
               frameon=False, title="Slide", fontsize=8)
    plt.tight_layout(rect=[0.8,0.8,1,1])
    plt.savefig(os.path.join(OUT_DIR, f"UMAP30_ComBat_{region1}_vs_{region2}_bySlide.png"),
                dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.legend(bbox_to_anchor=(1.02,1), loc="upper left",
               frameon=False, title="Region")
    plt.tight_layout(rect=[0.8,0.8,1,1])
    plt.savefig(os.path.join(OUT_DIR, f"UMAP30_ComBat_{region1}_vs_{region2}_byRegion.png"),
                dpi=300, bbox_inches="tight")
    plt.close()
...
# Top 30 features merged box plot
...
# ---- CONFIG ----
ROOT = "/Volumes/My Passport/Spatial_Proteomics_data_final/TDA_NC"
OUT_DIR = os.path.join(ROOT, "rf_top30_features_lists")
os.makedirs(OUT_DIR, exist_ok=True)
...
for region1, region2 in REGION_PAIRS:
    comp_tag = f"{region1}_vs_{region2}"
    rf_csv = os.path.join(ROOT, "rf_results", f"RF_{region1}_vs_{region2}_feature_mdi_folds.csv")
    ...
    top30.insert(1, "comparison", comp_tag)

    # Save CSV for this comparison
    out_csv = os.path.join(OUT_DIR, f"Top30_{comp_tag}.csv")
    top30.to_csv(out_csv)
    print(f"\n=== {comp_tag} ===")
    print(top30[["rank", "comparison", "mean_importance"]])

    # Add to combined list
    top30_reset = top30.reset_index().rename(columns={"index": "feature"})
    all_top30_records.append(top30_reset)

# Save combined table
if all_top30_records:
    combined = pd.concat(all_top30_records, ignore_index=True)
    combined_out = os.path.join(OUT_DIR, "Top30_features_all_comparisons.csv")
    combined.to_csv(combined_out, index=False)
    print(f"\nSaved combined top-30 table: {combined_out}")
...
# ---- CONFIG ----
ROOT = "/Volumes/My Passport/Spatial_Proteomics_data_final/TDA_NC"
COMBINED_CSV = os.path.join(ROOT, "rf_top30_features_lists", "Top30_features_all_comparisons.csv")

# ---- Load Combined Table ----
df = pd.read_csv(COMBINED_CSV)

# ---- Get union of all top-30 features across all comparisons ----
unique_features = sorted(df["feature"].unique())

# ---- Print the list and the count ----
print("\n=== Unique features in any Top-30 (union across all comparisons) ===")
for f in unique_features:
    print(f)
print(f"\nTotal: {len(unique_features)} unique features appeared in the top-30 for any comparison.")
...
# --- CONFIG ---
PLOT_DIR = "/Volumes/My Passport/Spatial_Proteomics_data_final/TDA_NC/boxplots_per_feature"
OUT_PDF  = "/Volumes/My Passport/Spatial_Proteomics_data_final/TDA_NC/combined_top_features_boxplots_A4.pdf"

features = [
    # ... 44 features list ...
    "Cluster_0_H0_significant_lifespan_mean",
    "Cluster_0_H0_significant_lifespan_std",
    "Cluster_0_H0_significant_nBars",
    "Cluster_0⟷Cluster_5_H0_significant_lifespan_mean",
    "Cluster_0⟷Cluster_5_H0_significant_lifespan_std",
    "Cluster_0⟷Cluster_5_H0_significant_nBars",
    "Cluster_0⟷Cluster_5_H1_significant_birth_mean",
    "Cluster_0⟷Cluster_5_H1_significant_lifespan_mean",
    "Cluster_0⟷Cluster_5_H1_significant_nBars",
    "Cluster_0⟷Cluster_7_H0_significant_lifespan_mean",
    "Cluster_0⟷Cluster_7_H0_significant_lifespan_std",
    "Cluster_0⟷Cluster_7_H0_significant_nBars",
    "Cluster_0⟷Cluster_7_H1_significant_birth_mean",
    "Cluster_0⟷Cluster_7_H1_significant_birth_std",
    "Cluster_0⟷Cluster_7_H1_significant_lifespan_mean",
    "Cluster_0⟷Cluster_7_H1_significant_nBars",
    "Cluster_4_H0_significant_lifespan_mean",
    "Cluster_4⟷Cluster_7_H0_significant_lifespan_mean",
    "Cluster_4⟷Cluster_7_H0_significant_lifespan_std",
    "Cluster_4⟷Cluster_7_H0_significant_nBars",
    "Cluster_4⟷Cluster_7_H1_significant_birth_mean",
    "Cluster_4⟷Cluster_7_H1_significant_birth_std",
    "Cluster_4⟷Cluster_7_H1_significant_lifespan_mean",
    "Cluster_4⟷Cluster_7_H1_significant_lifespan_std",
    "Cluster_4⟷Cluster_7_H1_significant_nBars",
    "Cluster_5_H0_significant_lifespan_mean",
    "Cluster_5_H0_significant_lifespan_std",
    "Cluster_5_H0_significant_nBars",
    "Cluster_5⟷Cluster_7_H0_significant_lifespan_mean",
    "Cluster_5⟷Cluster_7_H0_significant_lifespan_std",
    "Cluster_5⟷Cluster_7_H0_significant_nBars",
    "Cluster_5⟷Cluster_7_H1_significant_birth_mean",
    "Cluster_5⟷Cluster_7_H1_significant_birth_std",
    "Cluster_5⟷Cluster_7_H1_significant_lifespan_mean",
    "Cluster_5⟷Cluster_7_H1_significant_lifespan_std",
    "Cluster_5⟷Cluster_7_H1_significant_nBars",
    "Cluster_7_H0_significant_lifespan_mean",
    "Cluster_7_H0_significant_lifespan_std",
    "Cluster_7_H0_significant_nBars",
    "Cluster_7_H1_significant_birth_mean",
    "Cluster_7_H1_significant_birth_std",
    "Cluster_7_H1_significant_lifespan_mean",
    "Cluster_7_H1_significant_lifespan_std",
    "Cluster_7_H1_significant_nBars"
]

# --- LAYOUT ---
plots_per_page = 16      # 4x4 grid per A4
ncols, nrows = 4, 4
a4_inches = (8.27, 11.69)   # A4 size in inches

with PdfPages(OUT_PDF) as pdf:
    for page_start in range(0, len(features), plots_per_page):
        fig, axes = plt.subplots(nrows, ncols, figsize=a4_inches)
        axes = axes.flatten()
        for i in range(plots_per_page):
            ...
            if idx >= len(features):
                ...
                continue
            feat = features[idx]
            fname = feat.replace("⟷", "_") + ".png"
            fpath = os.path.join(PLOT_DIR, fname)
            if os.path.isfile(fpath):
                img = mpimg.imread(fpath)
                ax.imshow(img)
            else:
                ax.text(0.5, 0.5, "NOT FOUND", fontsize=8, ha="center", va="center")
            ax.axis('off')
        # Adjust vertical (hspace) and horizontal (wspace) margins
        fig.subplots_adjust(
            hspace=0,   # more space between rows
            wspace=0,   # more space between columns
            left=0.02,
            right=0.98,
            top=0.99,
            bottom=0.01
        )
        pdf.savefig(fig, dpi=350)  # REMOVE bbox_inches='tight'
        plt.close(fig)
