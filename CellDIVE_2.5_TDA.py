...
for r1, r2 in REGION_PAIRS:
    ...
    neg_feats = sorted(neg_feats, key=lambda x: deltas[x])
    pos_feats = sorted(pos_feats, key=lambda x: deltas[x])
    ...
    if pos_feats:
        ax.legend(pos_handles, pos_feats, title=r2,
                  loc='upper left', bbox_to_anchor=(1.30,1), frameon=False)
